...
class CronController extends Controller
{
    ...
    public function birthdaySchedule(): void
    {
        EmailContact::whereDay('birthdate', now()->day)
        ->whereMonth('birthdate', now()->month)
        ->chunk(20, function ($birthdayContacts) {
            foreach ($birthdayContacts as $contact) {
                $this->sendBirthdayEmail($contact);
            }
        });
    }

    public function sendBirthdayEmail($contact): void
    {
        EmailContact::chunk(200, function ($contacts) {
            $emailTemplate = EmailTemplates::where('name', 'BirthDay')->first();
            $now = Carbon::now();

            foreach ($contacts as $person) {
                $timezone = getTimezone($person->city);

                $birthdate = Carbon::createFromFormat('Y-m-d', $person->birthdate, $timezone);
                $currentTime = Carbon::now($timezone);

                if ($birthdate->isBirthday() && $currentTime->hour >= env('HOUR_SEND', 9)) {
                    $emailLog = EmailLog::where('to', $person->email)
                        ->where('sender_id', 14)
                        ->where('status', '!=', 3)
                    ...
                    if (!$emailLog) {
                        $emailLog = new EmailLog([
                            'sender_id' => 14,
                            'from_name' => 'vanika@unika.ac.id',
                            'reply_to_email' => 'vanika@unika.ac.id',
                            'to' => $person->email,
                            'message' => $emailTemplate->body ?? "Happy birthday {{username}}",
                            'subject' => 'Happy Birth Day ' . $person->name . ' From ' . $person->city,
                            'status' => 2,
                            'schedule_status' => 2,
                            'initiated_time' => $now->addMinutes(5),
                        ]);
                        $emailLog->save();
                    }
                }
    ...
    public function processWhatsappCampaign($campaign): void
    {
        ...
        $flag            = 1;
        $whatsappGateway = WhatsappDevice::whereNull('user_id')->where('status', 'connected')->select('max_delay','min_delay','id')->get();


        if($campaign->user_id){
            $user = User::where('id',$campaign->user_id)->first();
            if($user){
                $whatsappGateway = WhatsappDevice::where('user_id', $user->id)->where('status', 'connected')->select('max_delay','min_delay','id')->get();
        ...
        if($flag == 1){
            ...
            foreach($contacts  as $contact){
                if(filter_var($contact->contact, FILTER_SANITIZE_NUMBER_INT)){
                    ...
                    $log->contact_id  = $contact->id;
                    if($user) {
                        $log->api_gateway_id = auth()->user()->sms_gateway == 1 ? $smsGateway->id : null;
                    }else{
                        $log->api_gateway_id = null;
                        if($general->sms_gateway == 1){
                            $log->api_gateway_id = (int) $smsGateway->id;
                        }
                    }
